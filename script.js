...
window.particlesJS = function (body, params) {
  //console.log(params);
  ...
  /* pJS elements */
  var pJS_tag = document.getElementById(body),
  ...
  /* append canvas */
  var canvas = document.getElementById(body).appendChild(canvas_el);

  /* launch particle.js */
  if (canvas != null) {
    pJSDom.push(new pJS(body, params));
  }
};

window.particlesJS.load = function (body, path_config_json, callback) {
  /* load json config */
  var xhr = new XMLHttpRequest();
  ...
  xhr.onreadystatechange = function (data) {
    if (xhr.readyState == 4) {
      if (xhr.status == 200) {
        ...
        window.particlesJS(body, params);
